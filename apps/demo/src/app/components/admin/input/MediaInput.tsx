...
export const MediaInput = ({
	...
	return (
		...
		>
			...
			<div
				className={`flex justify-between h-[176px] gap-2 rounded-lg border border-dashed border-text border-opacity-50 transition-colors ${
          ...
          `}
			...
			{error && (
				<p className="pt-1 text-2xs normal-case text-red-500">{error}</p>
			)}
		</div>
	);
};
...
export const BlurImage = (props: ComponentProps<typeof Image>) => {
	...
	return (
      ...
      ${
				isLoading
					? "scale-105 animate-pulse bg-slate-400 blur-lg"
					: "scale-100 blur-0"
			}
      `}
