...
export const Select = ({
	...
	return (
		<Listbox name={id} value={selected} onChange={handleSelected} defaultValue={placeholder}>
			<div className={`relative ${fullWidth ? "w-full" : ""}`}>
				<Listbox.Button
					className={`relative flex cursor-pointer items-center justify-between gap-2 border bg-secondary backdrop-blur-sm border-secondary px-4 py-2 h-[63px] ${fullWidth ? "w-full" : ""}`}>
					<span className={`block truncate ${emptyValue ? "text-placeholder" : ""}`}>
