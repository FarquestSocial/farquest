...
import { IAdminQuest, IRewardType } from "@/utils/types";
import { routes } from "@/constants/admin-routes";
import { QuestSchema, questSchema } from "@/schemas/quest-schema";
import {
...
export const QuestModal = ({
  ...
  return (
    ...
    >
      ...
      <ModalBody
        className={`px-5 ${!selectedQuestType?.id && !isEditMode ? "" : "overflow-y-auto"}`}
      >
        <form>
            ...
            <div className='flex flex-col gap-4'>
              ...
              selectedQuestType?.questType !== "TWITTER_ENGAGEMENT" ? (
                <Input
                  ...
                  value={xpAmount}
                  onChange={(e) => {
                    setValue("rewardXp", parseInt(e.target.value));
                  }}
                />
              ) : null}
              <Input
      ...
      <ModalFooter>
        <section className='flex gap-x-4'>
          ...
          <GeneralButton
            ...
            className={`text-lg capitalize bg-yellow bg-primary ${isSubmitting ? "animate-pulse" : ""}`}
            disabled={isSubmitting}
          >
