...
export class FollowQuestHandler {
    ...
    async completeQuest(userId: string, questId: string): Promise<boolean> {
      console.log(`FollowQuestHandler: Starting quest completion for user ${userId} and quest ${questId}`);
      ...
      try {
        ...
        if (!validationCriteria || !validationCriteria.farCastId) {
          console.error(`No or invalid validation criteria found for quest ${questId}`);
          return false;
        }
  
        // Check if the user has followed the specified farCastId
        const hasFollowed = await this.airStackService.hasUserFollowedUser(userId, validationCriteria.farCastId);
        if (!hasFollowed) {
          console.log(`User ${userId} has not followed the required user ${validationCriteria.farCastId} for quest ${questId}`);
          return false;
        }
        ...
        console.error(`Error completing quest for user ${userId} on quest ${questId}: ${error}`);
