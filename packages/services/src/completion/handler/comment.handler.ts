...
export class CommentQuestHandler {
    ...
    async completeQuest(userId: string, questId: string): Promise<boolean> {
      console.log(`CommentQuestHandler: Starting quest completion for user ${userId} and quest ${questId}`);
      ...
      try {
        ...
        const hasCommented = await this.airStackService.hasUserCommentedOnCast(userId, validationCriteria.castId);
        if (!hasCommented) {
          console.log(`User ${userId} has not commented on the required cast for quest ${questId}`);
          return false;
        }
